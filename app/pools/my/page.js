...
function MyPitchesContent() {
    ...
    const handleTabChange = (newTab) => {
        setTab(newTab);
        setFilter('ALL');
        router.push(`/pools/my?tab=${newTab}`, { scroll: false });
    };
    ...
    return (
        <>
                                        ...
                                        <div key={p.id} className={`${styles.savedCard} ${isExpired ? styles.savedCardExpired : ''} ${isCompleted ? styles.savedCardCompleted : ''}`}>
                                            ...
                                            <div className={styles.savedBody}>
                                                ...
                                                <div className={styles.savedHostRow}>
                                                    ...
                                                    <button className={`${styles.savedBtnView} ${isExpired ? styles.savedBtnViewExpired : ''}`} onClick={() => router.push(`/pools/${p.id}`)}>
                                                        Details
                                                    </button>
                                                </div>
                                            </div>
                                        ...
                                        <div key={p.id} className={`${styles.savedCard} ${isExpired ? styles.savedCardExpired : ''} ${isCompleted ? styles.savedCardCompleted : ''}`}>
                                            ...
                                            <div className={styles.savedBody}>
                                                ...
                                                <div className={styles.hostFooterRow}>
                                                    {isDraft ? (
                                                        <button className={styles.hostEditDraftBtn} onClick={() => router.push(`/pools/create?draftId=${p.id}`)}>Edit Draft</button>
                                                    ) : (
                                                        <>
                                                            ...
                                                            <div className={styles.hostBtnGroup}>
                                                                <button className={styles.hostViewBtn} onClick={() => router.push(`/pools/${p.id}`)}>View Details</button>
                                                                <button className={`${styles.savedBtnView} ${isCompleted ? styles.hostManageMuted : ''}`} onClick={() => router.push(`/pools/${p.id}/host-dashboard`)}>Manage</button>
                                                            </div>
                                                        </>
                                                    )}
                                                </div>
                        ...
                        return (
                            <div className={styles.savedGrid}>
                                {filteredSaved.map(p => {
                                    ...
                                    const isCompleted = p.savedStatus === 'completed';
                                    ...
                                    if (isExpired) {
                                        ...
                                        badgeCls = styles.savedBadgeCompleted;
                                        statusLabel = 'COMPLETED';
                                    } else if (p.savedStatus === 'order_placed') {
                                        badgeCls = styles.savedBadgeActive;
                                        statusLabel = 'ORDER PLACED';
                                    }

                                    return (
                                        <div key={p.id} className={`${styles.savedCard} ${isExpired ? styles.savedCardExpired : ''}`}>
                                            <div className={`${styles.savedImageWrap} ${isExpired ? styles.savedImageGrayscale : ''}`}>
                                                {p.image ? (
                                                    <img src={p.image} alt={p.title} className={styles.savedImage} />
                                                ) : (
                                            ...
                                            <div className={styles.savedBody}>
                                                ...
                                                <div className={styles.savedGoalSection}>
                                                    <div className={styles.savedGoalHeader}>
                                                        <span>{isExpired ? 'GOAL NOT MET' : isCompleted ? 'FULLY FUNDED' : 'PROGRESS'}</span>
                                                        <span className={isExpired ? styles.savedGoalPctMuted : styles.savedGoalPct}>
                                                            {isExpired ? `${p.committedUnits || 2}/${p.minOrder || 4} (EXPIRED)` : `${p.committedUnits || 3}/${p.minOrder || 5} GOAL`}
                                                        </span>
                                                    </div>
                                                    <div className={styles.savedGoalTrack}>
                                                        <div className={`${styles.savedGoalFill} ${isExpired ? styles.savedGoalFillMuted : ''}`} style={{ width: `${p.goalPercent}%` }} />
                                                    </div>
                                                </div>
                                                <div className={styles.savedHostRow}>
                                                    <div className={styles.savedHostInfo}>
                                                        <div className={styles.savedHostAvatarWrap}>
                                                            <Avatar name={p.hostName || p.host?.name || 'Community Host'} src={p.hostAvatar || p.host?.avatarUrl} size="sm" className={isExpired ? styles.savedImageGrayscale : ''} />
                                                        </div>
                                                        <div>
                                                            <p className={styles.savedHostLabel}>HOST</p>
                                                            <p className={`${styles.savedHostName} ${isExpired ? styles.savedTitleMuted : ''}`}>
                                                                {p.hostName || p.host?.name || 'Community Host'}
                                                            </p>
                                                        </div>
                                                    </div>
                                                    {isExpired ? (
                                                        ...
                                                        <button className={styles.savedBtnView} onClick={() => router.push(`/pools/${p.id}`)}>View Details</button>
                                                    )}
                                                </div>
                                            </div>
                                        </div>
            ...
            {/* FAB — Create Pitch */}
            <button className={styles.fab} style={{ bottom: 'calc(75px + var(--space-6))', zIndex: 10 }} onClick={() => router.push('/pools/create')}>
